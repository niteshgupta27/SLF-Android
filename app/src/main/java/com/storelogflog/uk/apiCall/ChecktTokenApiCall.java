...
public class ChecktTokenApiCall {
    String TAG = this.getClass().getSimpleName();
    Context mContext;
    VolleyApiResponseString volleyApiResponseString;

    public ChecktTokenApiCall(Context mContext, VolleyApiResponseString volleyApiResponseString, String token, int serverCode) {
    ...
    private void callApi(final String token, final int serverCode) {
        ...
        Logger.debug(TAG,""+Constants.BASE_URL+Constants.API_CHECK_TOKEN);
        Logger.debug(TAG,"Request_Token="+token);

        StringRequest apiRequest = new StringRequest(Request.Method.POST, Constants.BASE_URL+Constants.API_CHECK_TOKEN, new Response.Listener<String>(){
