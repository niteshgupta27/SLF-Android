...
public class ViewItemApiCall {
    String TAG = this.getClass().getSimpleName();
    Context mContext;
    VolleyApiResponseString volleyApiResponseString;

    public ViewItemApiCall(Context mContext, VolleyApiResponseString volleyApiResponseString, String token, int serverCode) {
    ...
    private void callApi(final String token, final int serverCode) {
        ...
        Logger.debug(TAG,""+Constants.BASE_URL+Constants.API_VIEW_ITEM);
        Logger.debug(TAG,"Request_Token="+token);

        StringRequest apiRequest = new StringRequest(Request.Method.POST, Constants.BASE_URL+Constants.API_VIEW_ITEM, new Response.Listener<String>(){
