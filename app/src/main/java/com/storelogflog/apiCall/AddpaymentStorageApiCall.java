...
public class AddpaymentStorageApiCall {
    String TAG = this.getClass().getSimpleName();
    Context mContext;
    VolleyApiResponseString volleyApiResponseString;

    public AddpaymentStorageApiCall(Context mContext, VolleyApiResponseString volleyApiResponseString, String token, int serverCode) {
    ...
    private void callApi(final String token, final int serverCode) {
        ...
        Logger.debug(TAG,""+Constants.BASE_URL+Constants.API_PaymentStorge);
        Logger.debug(TAG,"Request_Token="+token);

        StringRequest apiRequest = new StringRequest(Request.Method.POST, Constants.BASE_URL+Constants.API_PaymentStorge, new Response.Listener<String>(){
