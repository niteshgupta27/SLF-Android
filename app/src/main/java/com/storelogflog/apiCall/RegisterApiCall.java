...
import com.storelogflog.activity.RegisterActivity;
...
public class RegisterApiCall {
    ...
    public RegisterApiCall(Context mContext, VolleyApiResponseString volleyApiResponseString,String token,int serverCode) {
    ...
    private void callApi(final String token, final int serverCode) {
        ...
        Logger.debug(TAG,""+Constants.BASE_URL+Constants.API_REGISTER);
        Logger.debug(TAG,"Request_Token="+token);

        StringRequest apiRequest = new StringRequest(Request.Method.POST, Constants.BASE_URL+Constants.API_REGISTER, new Response.Listener<String>(){
